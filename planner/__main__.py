...
if __name__ == "__main__":
    dir_inputs = os.path.abspath(os.path.join(dir_parent_1,'inputs'))  # 输入位置
    dir_outputs = os.path.abspath(os.path.join(dir_parent_1,'outputs'))  # 输出位置
    dir_save_img = os.path.abspath(os.path.join(dir_parent_1,'onsite_images_saved'))  # 可视化保存位置
    tic = time.time()  # 开始时间
    so = scenarioOrganizer.ScenarioOrganizer()  # 待测场景管理器实例化
    envi = env.Env()  # 仿真环境实例化
    # 根据配置文件config.py装载场景,指定输入文件夹即可,会自动检索配置文件
    so.load(dir_inputs,dir_outputs)  # 加载待测环境
    formatted_so_config = json.dumps(so.config, indent=4, ensure_ascii=False)  # 清晰展示各种设置参数
    print(f"###log### <测试参数>\n{formatted_so_config}\n")
    ...
    while True:
        ...
        observation,traj = envi.make(scenario=scenario_to_test,collision_lookup = collision_lookup, save_img_path=dir_save_img)  # 初始化环境
