...
class Predictor:
    ...
    def predict(self,observation:observation.Observation=None,traj:List=None,predictor_type="CACV_PREDICTOR")-> List[Dict]:
        """ 调用轨迹预测器,进行目标车辆轨迹预测.
        ...
        if self.predictor_type_ == "CACV_PREDICTOR" :
            ...
            for id_veh in observation['vehicle_info'].keys():
                ...
                if id_veh not in self.veh_predictor_info:# 第一次 出现id_veh车 & 第一次匹配到dubinspose,以后一直使用该dubinspose start
                    ...
                    veh_match_info_first = {"match_info_time":observation['test_setting']['t'],
                                            "polygon_token":polygon_token,
                                            "token_dubinspose":token_dubinspose_start,
                                            "ref_path_tokens":ref_path_tokens} # ! 注意:ref_path_tokens 确定后不可更改                        
                ...
                _,longitudinal_error,_=utils.compute_two_pose_error(veh_x,veh_y,veh_yaw,
                ...
                if self.veh_predictor_info[id_veh]["predictor_type_update"] == "CACV_PREDICTOR":
                    ...
                    if "CACV_PREDICTOR" not in self.veh_predictor_info[id_veh]:# 第一次 给id_veh车初始化CACV_PREDICTOR预测器              
                        self.veh_predictor_info[id_veh]["CACV_PREDICTOR"]= ConstantAngularVelocityPredictor(t_now_,interval_,
                                                                                max_t_,self.prediction_total_time_)
                    ...
                    if "LANE_SEQUENCE_PREDICTOR" not in self.veh_predictor_info[id_veh]:# 第一次 给id_veh车初始化LANE_SEQUENCE_AND_CACV预测器              
                        self.veh_predictor_info[id_veh]["LANE_SEQUENCE_PREDICTOR"]= LaneSequencePredictor(t_now_,interval_,
                                                                                            max_t_,self.prediction_total_time_)
                    
                    
                    # 车辆匹配信息实时更新(部分1)
                    veh_match_info_update= {"match_info_time":observation['test_setting']['t'],
                                            "polygon_token":polygon_token,
                    ...
                    traj_future_,traj_assesment_,paths_prob_info,flag_preview_dis_over_end= self.veh_predictor_info[id_veh]["LANE_SEQUENCE_PREDICTOR"].predict(
...
class ConstantAngularVelocityPredictor:
    ...
    def predict(self,traj_all:Dict,t_now_:float)->Dict:
        ...
        for i in range( self.numOfTrajPoint_):
            ...
            self.traj_predict_[str_time] = {'x':round(x,2),'y':round(y,2),
                                         'v':round(v,2),'a':round(acc,2),'yaw':round(yaw,3)}
...
class LaneSequencePredictor:
    ...
    def predict(self,traj_now:Dict,traj_future_1s:Dict,observation:observation.Observation,
                veh_match_info_first:Dict,veh_match_info_update:Dict ):
        """调用轨迹预测器,进行目标车辆轨迹预测
        ...
        if flag_preview_dis_over_end == True:#目标车辆太靠近 dubinspose end,需要返回flag,调用CACV预测器
            ...
            return self.veh_traj_predicted,traj_assesment_,paths_prob_info,flag_preview_dis_over_end
        
        # 4.2)进行多项式轨迹生成
        pt1=[self.x,self.y]
        pt2=[path_points[target_path_point_id][0],path_points[target_path_point_id][1]]
        v1 = [self.v*np.cos(self.yaw),self.v*np.sin(self.yaw)]
        v2 = [self.v*np.cos(path_points[target_path_point_id][2]),self.v*np.sin(path_points[target_path_point_id][2])]
        a1 = [0.0,0.0]
        a2 = [0.0,0.0]
        ...
        for i in range(len(interpolation_trajectory)):
            ...
            acc = self.acc
            yaw = self.yaw
            self.veh_traj_predicted[str_time] = {'x':round(x,2),'y':round(y,2),
                                         'v':round(v,2),'a':round(acc,2),'yaw':round(yaw,3)} 
    ...
    def _reference_path_evaluator(self,traj_future_1s:Dict,observation:observation.Observation)->list:
        ...
        for path_token in self.veh_match_info_first['ref_path_tokens']:
            ...
            similarity_score.append([path_token,nearest_id,score,similarity_euclidean_distance,similarity_deltaYaw,0.0])
            sum_value += score 
    ...
    def _cal_similarity_score(self,path_token:str,traj_future_1s:dict,observation:observation.Observation):
        ...
        euclidean_distance = math.sqrt((self.x - path_points[nearest_id][0])**2 + 
                                       (self.y - path_points[nearest_id][1])**2)
        ...
        return nearest_id,similarity_score,similarity_euclidean_distance,similarity_deltaYaw
    ...
    def _generate_traj_predicted(self,ref_path_XYHeadingS):
        ...
        dist_predic_space_max = ref_path_XYHeadingS[-1][4] - ref_path_XYHeadingS[self.road_nearest_point_index][4]# 当前时刻该 ref_path 还剩下的距离,作为最大预测距离
        self.traj_predict_ = {}
        for i in range( self.numOfTrajPoint_):
            t += dt
            ...
            self.traj_predict_[str_time] = self._generate_traj_predicted_sub(ref_path_XYHeadingS,
                                                                          sum_distance_1 + 
                                                                          ref_path_XYHeadingS[self.road_nearest_point_index][4])
    ...
    def _generate_traj_predicted_sub(self,ref_path_XYHeadingS,sum_distance):
        ...
        traj_pred_sub_= {'x':round(x,2),'y':round(y,2),'v':round(v,2),'a':round(acc,2),'yaw':round(yaw,3)}
